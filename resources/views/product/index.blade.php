                                ...
                                <div class="col-md-4 py-3">
                                    <div class="card">
                                        <img style="width: 100%; height: 250px; cursor: pointer;"
                                            src="{{ asset($product->image) }}" class="card-img-top img-fluid openProduct"
                                            alt="..." data-id="{{ $product->id }}">
                                        <div class="card-body">
                                            <h5 class="fw-semibold fs-4">{{ $product->name }}</h6>
                                                <div>
                                                    <h6 class="fs-4 mb-0"><i class="ti ti-building-store"></i>
                                                        {{ $product->store_name }}</h6>
                                                    <p class="card-text"><span class="ti ti-currency-peso"></span>
                                                        {{ $product->price }}</p>
                                                </div>
                                        </div>
                                    </div>
                    ...
                    $("#submitProductModalButton").click(function() {
                        ...
                        $.ajax({
                            ...
                            error: function(response) {
                                ...
                                $('#product_price_error').text(errors.product_price ?
                                    errors.product_price[0] : '');
                                $('#product_name_error').text(errors.product_name ?
                                    errors.product_name[0] : '');
                                $('#product_description_error').text(errors
                                    .product_description ? errors
                                    .product_description[0] : '');
                                $('#product_image_error').text(errors.product_image ?
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $.each(response.data, function(dataIndex, dataValue) {
                            ...
                            $('.productImagePreview').attr('src', logo).css('display',
                                'block');
            ...
            $("#updateProduct").click(function() {
                ...
                $.ajax({
                    ...
                    error: function(response) {
                        ...
                        let errors = response.responseJSON.errors;
                        $('#product_price_error').text(errors.product_price ? errors
                            .product_price[0] : '');
                        $('#product_name_error').text(errors.product_name ? errors.product_name[
                            0] : '');
                        $('#product_description_error').text(errors.product_description ? errors
                            .product_description[0] : '');
                        $('#product_image_error').text(errors.product_image ? errors
                            .product_image[0] : '');
                    }
                })
            });
